...
DATA_FILE = DATASET_DIR / "crossword_finetune_20251125_200641.jsonl"
...
if LOCAL_MODEL_DIR.exists() and (LOCAL_MODEL_DIR / "config.json").exists():
    ...
    if not hf_token:
        ...
        print("   c) Przekaż token jako argument (zostanie dodane w następnej wersji)")
...
# Output directory
OUTPUT_DIR = SCRIPT_DIR / "bielik-crossword-qlora-1k"
...
def main():
    ...
    # 5. Parametry treningu (na start ostrożnie)
    training_args = SFTConfig(
        output_dir=str(OUTPUT_DIR),
        num_train_epochs=1,                    # 1 epoka na próbę
